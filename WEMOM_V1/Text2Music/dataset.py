...
class MapDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Remove the extra dimension introduced by torch.cat with unsqueeze inside list comprehension
        # If batch_size > 1 in DataLoader, this will add another dimension, so we want
        # pos_text to be [L, feature_dim] where L is number of positives for this anchor.
        # But wait, DataLoader collate_fn will stack them.
        
        # The issue typically arises if each sample returns [1, dim] instead of [dim].
        # Let's inspect shapes. Assuming each line in text file has just ONE positive/negative for simplicity
        # or we treat them as fixed length. 
        
        # Correction: The original code logic seems to assume one sample has multiple positives/negatives.
        # pos_text shape: [num_pos, 128]
        
        # IF DataLoader batch_size > 1:
        # Batch Output: [Batch, num_pos, 128]

...
with open('D:/Projects/URIS/URIS/WEMOM_V1/EmoText/TextVAE_config.json') as f:
    args = json.load(f)

id2word = np.load('D:/Projects/URIS/URIS/WEMOM_V1/Data/Saves/id2word_text.npy', allow_pickle=True).item()
...
save_path = 'D:/Projects/URIS/URIS/WEMOM_V1/EmoText/params/'
...
def loss_function(logit, text_target, mean, std, logits, labels, KL_weight, class_weights=class_weights):
    ...
    # Increase classification weight relative to other losses significantly
    # Because valid_accuracy is our main metric
    ...
    # Balance: Reconstruction is usually ~7.0, KL ~1.0, Classification ~1.5
    # We want Classification to drive the updates.
...
def evaluation_phase():
    ...
    with torch.no_grad():
        for sentence, labels in test_data:
            ...
            # Fix: Use same KL weight as training config (args['lambda']) instead of 1.0
            loss, reconstruction_loss, kl_loss, classification_loss = loss_function(
                logit, target_output, mean, std, logits, labels, KL_weight=args['lambda'], class_weights=class_weights)
    ...
    # Detailed print
    print(f"Validation Loss: {total_loss / len(test_data):.4f} (Rec: {reconstruction_loss.item():.4f}, KL: {kl_loss.item():.4f}, Cls: {classification_loss.item():.4f}), Accuracy: {accuracy:.4f}")
